...
const TranscriptionsDataView = ( { transcriptions, onSavingTranscription, onDeletingTranscription }) => {
    ...
    const actions=[
        {
          RenderModal: (item) => { 
            ...
            return <ConfirmDeleteTranscription 
                        fileName={item.items[0].file_id} 
                        onConfirm={handleConfirmDelete} 
                        onCancel={handleCancelDelete} 
                        closeModal={item.closeModal} 
                        videoTitle={item.items[0].transcription.videoTitle}
                        /> 
          },
    ...
    const fields = [
        ...
        // },
        {
            ...
            render: ( { item } ) => {
                return (
                    <>
                    { item.uploaded === undefined || item.uploaded
                        ? <OkSvg style={{ width: "24px", height: "24px", position: "absolute" }}/>
                        : <ToBeUploadedSvg style={{ width: "24px", height: "24px", position: "absolute" }}/> }
                    </>
                );
            }
        }
    ...
    const shouldEnableUpload = useMemo(() => {
        try {
            return selectedTranscription && selectedTranscription.file_id.startsWith('file-');
        } catch {
            return false;
        }
    ...
    return (
        <>
            <Panel header="Importa Trascrizioni" className='flex-none m-1 h-5/12 w-[99%]'>  
